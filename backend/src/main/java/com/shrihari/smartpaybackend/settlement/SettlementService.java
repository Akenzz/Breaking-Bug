...
public class SettlementService {
    ...
    public SettlementInitiateResponse initiateSettlement(
        ...
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new ApiException("Settlement amount must be positive");
        }
        ...
        User fromUser = userRepository.findById(fromUserId)
                .orElseThrow(() -> new ApiException("From user not found"));

        User toUser = userRepository.findById(toUserId)
                .orElseThrow(() -> new ApiException("To user not found"));
...
//        riskService.evaluateAndBlock(fromUser, toUser, amount);
    ...
    public void claimPaid(Long settlementId) {
        ...
        Settlement settlement = settlementRepository.findById(settlementId)
                .orElseThrow(() -> new ApiException("Settlement not found"));

        if (!settlement.getFromUser().getId().equals(currentUser.getId())) {
            throw new ApiException("Only the payer can claim this settlement");
        }

        if (settlement.getStatus() != SettlementStatus.PENDING) {
            throw new ApiException("Settlement already processed");
        }
    ...
    public void confirmSettlement(Long settlementId) {
        ...
        if (!settlement.getToUser().getId().equals(currentUser.getId())) {
            throw new ApiException("Only the receiver can confirm this settlement");
        }

        if (settlement.getStatus() != SettlementStatus.PENDING_CONFIRMATION) {
            throw new ApiException("Settlement is not pending confirmation");
        }
    ...
    public void disputeSettlement(Long settlementId) {
        ...
        if (!settlement.getToUser().getId().equals(currentUser.getId())) {
            throw new ApiException("Only the receiver can dispute this settlement");
        }
